...
public class HistoryShareHousingPhotoRecyclerViewAdapter extends RecyclerView.Adapter<HistoryShareHousingPhotoRecyclerViewAdapter.ViewHolder> {

    public static final String TAG = HistoryShareHousingPhotoRecyclerViewAdapter.class.getSimpleName();
    ...
    public HistoryShareHousingPhotoRecyclerViewAdapter(Context context, List<HistoryShareHousingPhoto> items, OnShareHousingListInteractionListener listener) {
        mContext = context;
        mHistoryShareHousingPhotos = items;
        mListener = listener;
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        holder.mItem = mHistoryShareHousingPhotos.get(position).getShareHousing();
        if (!mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getPhotoURLs().isEmpty()) {
            String profileImageUrl = mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getPhotoURLs().get(0);
        ...
        if (!TextUtils.isEmpty(mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getTitle())) {
            holder.mHouseTitle.setText(mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getTitle());
        }
        if (!TextUtils.isEmpty(mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getHouseType())) {
            holder.mHouseType.setText(mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getHouseType());
        }
        if (!TextUtils.isEmpty(mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getAddressDistrict())
                && !TextUtils.isEmpty(mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getAddressCity())) {
            holder.mHouseLocation.setText(
//                    mContext.getString(R.string.housing_recycler_view_adapter_house_location) +
                    mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getAddressDistrict() + ", "
                    + mHistoryShareHousingPhotos.get(position).getShareHousing().getHousing().getAddressCity()
            );
        }

//        holder.mNumSaved.setText(String.valueOf(mHistoryShareHousingPhotos.get(position).getNumOfSaved()) + mContext.getString(R.string.housing_recycler_view_adapter_saved));

        Pair<String, String> pair = HousePriceHelper.parseForShareHousing(mHistoryShareHousingPhotos.get(position).getShareHousing().getPricePerMonthOfOne(), mContext);
        if (pair.first == null) {
        ...
        if (!TextUtils.isEmpty(mHistoryShareHousingPhotos.get(position).getShareHousing().getRequiredGender())) {
            holder.mRequiredGender.setText(mHistoryShareHousingPhotos.get(position).getShareHousing().getRequiredGender());
        } else {
            holder.mRequiredGender.setText(mContext.getString(R.string.hint_signup_gender_female));
        }

        if (mHistoryShareHousingPhotos.get(position).getShareHousing().getRequiredNumOfPeople() <= 0) {
            holder.mRequiredNumOfPeople.setText(1 + mContext.getString(R.string.activity_housing_detail_people));
        } else {
            holder.mRequiredNumOfPeople.setText(mHistoryShareHousingPhotos.get(position).getShareHousing().getRequiredNumOfPeople() + mContext.getString(R.string.activity_housing_detail_people));
        }

        if (!TextUtils.isEmpty(mHistoryShareHousingPhotos.get(position).getShareHousing().getRequiredWorkType())) {
            holder.mRequiredWorkType.setText(mHistoryShareHousingPhotos.get(position).getShareHousing().getRequiredWorkType());
        } else {
...
//                BitmapLoader.decodeSampledBitmapFromFile(localFile, 56, 56));
    ...
    public int getItemCount() {
        return mHistoryShareHousingPhotos != null ? mHistoryShareHousingPhotos.size() : 0;
    }
