...
public class HistoryHousingNoteRecyclerViewAdapter extends RecyclerView.Adapter<HistoryHousingNoteRecyclerViewAdapter.ViewHolder> {

    public static final String TAG = HistoryHousingNoteRecyclerViewAdapter.class.getSimpleName();

    private final Context mContext;
    private final List<HistoryHousingNote> mHistoryHousingNotes;
    private final OnHousingListInteractionListener mListener;

    public HistoryHousingNoteRecyclerViewAdapter(Context context, List<HistoryHousingNote> items, OnHousingListInteractionListener listener) {
        mContext = context;
        mHistoryHousingNotes = items;
        mListener = listener;
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        holder.mItem = mHistoryHousingNotes.get(position).getHousing();
        if (!mHistoryHousingNotes.get(position).getHousing().getPhotoURLs().isEmpty()) {
            String profileImageUrl = mHistoryHousingNotes.get(position).getHousing().getPhotoURLs().get(0);
        ...
        if (!TextUtils.isEmpty(mHistoryHousingNotes.get(position).getHousing().getTitle())) {
            holder.mHouseTitle.setText(mHistoryHousingNotes.get(position).getHousing().getTitle());
        }
        if (!TextUtils.isEmpty(mHistoryHousingNotes.get(position).getHousing().getHouseType())) {
            holder.mHouseType.setText(mHistoryHousingNotes.get(position).getHousing().getHouseType());
        }
        if (!TextUtils.isEmpty(mHistoryHousingNotes.get(position).getHousing().getAddressDistrict())
                && !TextUtils.isEmpty(mHistoryHousingNotes.get(position).getHousing().getAddressCity())) {
            holder.mHouseLocation.setText(
//                    mContext.getString(R.string.housing_recycler_view_adapter_house_location) +
                    mHistoryHousingNotes.get(position).getHousing().getAddressDistrict() + ", "
                    + mHistoryHousingNotes.get(position).getHousing().getAddressCity()
            );
        }

//        holder.mNumSaved.setText(String.valueOf(mHistoryHousingNotes.get(position).getNumOfSaved()) + mContext.getString(R.string.housing_recycler_view_adapter_saved));

        Pair<String, String> pair = HousePriceHelper.parseForHousing(mHistoryHousingNotes.get(position).getHousing().getPrice(), mContext);
        if (pair.first == null) {
...
//                BitmapLoader.decodeSampledBitmapFromFile(localFile, 56, 56));
    ...
    public int getItemCount() {
        return mHistoryHousingNotes != null ? mHistoryHousingNotes.size() : 0;
    }
